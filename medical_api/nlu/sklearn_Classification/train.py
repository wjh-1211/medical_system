...
def load_data(data_path):
    ...
    print("Total samples loaded:", len(X))  # 打印总数据量
...
def run(data_path,model_save_path):
    ...
    print("Training samples:", len(train_X))  # 打印训练集大小
    print("Testing samples:", len(text_X))  # 打印测试集大小
    ...
    # -------------LR--------------
    # 通过调整joblib中n_jobs参数，尝试将n_jobs设置为1或原有配置数-1。否则会报错，原先该参数为4
    LR = LogisticRegression(C=8, dual=False,n_jobs=1,max_iter=400,multi_class='ovr',random_state=122)
